    ...
    <div className="space-y-8 pb-40">
      <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
        <div className="relative z-10">
          <h2 className="text-3xl font-black uppercase italic tracking-tighter">Gestor de Mapeamento</h2>
          <p className="text-slate-400 font-bold text-sm mt-2">Clique no nome para editar ou use a busca para puxar novas APIs.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories?.map(category => {
          ...
          return (
            <Card key={category.id} className="border-slate-100 shadow-xl rounded-[2rem] bg-white flex flex-col relative overflow-visible">
              <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-6 rounded-t-[2rem] shrink-0">
              ...
              <CardContent className="p-4 flex-1 flex flex-col gap-4 overflow-visible">
                <div className="min-h-[150px] max-h-[300px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                  {categoryApis.map(api => (
                    <div key={api.id} className="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-2xl group hover:border-blue-200 transition-all">
                  ...
                  {categoryApis.length === 0 && (
                    <div className="h-32 flex flex-col items-center justify-center border-2 border-dashed border-slate-50 rounded-3xl text-slate-200 uppercase text-[10px] font-black">Vazio</div>
                  )}
                </div>
                ...
                <div className="relative mt-auto z-[50]">
                  ...
                  {currentSearch && (
                    <div className="fixed sm:absolute z-[100] left-0 w-full mt-2 bg-white border border-slate-200 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.2)] max-h-80 overflow-y-auto p-2 animate-in fade-in slide-in-from-top-2 ring-1 ring-black/5">
                      <div className="px-3 py-2 text-[8px] font-black text-slate-400 uppercase tracking-widest border-b mb-1">Resultados da Busca</div>
                      {apis?.filter(a => a.category_id !== category.id && (a.name.toLowerCase().includes(currentSearch.toLowerCase()) || a.endpoint.toLowerCase().includes(currentSearch.toLowerCase())))
                        .slice(0, 20).map(api => {
                          ...
                          return (
                            <div 
                              key={api.id}
                              className="p-3 hover:bg-blue-600 hover:text-white rounded-xl flex items-center justify-between cursor-pointer group/search mb-1 transition-colors"
                              onClick={() => {
                            ...
                            >
                              <div className="flex flex-col overflow-hidden">
                                <div className="flex items-center gap-2">
                                  ...
                                  {otherCat && <Badge className="bg-slate-100 text-slate-500 group-hover/search:bg-white/20 group-hover/search:text-white text-[7px] font-black px-1 h-3">{otherCat.icon} {otherCat.name}</Badge>}
                                </div>
                                <span className="text-[8px] font-mono opacity-50 truncate">{api.endpoint}</span>
                              </div>
